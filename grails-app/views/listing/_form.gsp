...
<div class="fieldcontain ${hasErrors(bean: listingInstance, field: 'seller', 'error')} required">
	...
	<g:select id="seller" name="seller.id" from="${myebay1.Customer.list()}" optionKey="id" required="" value="${listingInstance?.seller?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: listingInstance, field: 'description', 'error')} ">
	...
	<g:textArea name="description" cols="40" rows="5" maxlength="256" value="${listingInstance?.description}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: listingInstance, field: 'startingBidPrice', 'error')} required">
	...
	<g:field type="number" name="startingBidPrice" min="1.0" required="" value="${fieldValue(bean: listingInstance, field: 'startingBidPrice')}"/>
</div>

<div class="fieldcontain ${hasErrors(bean: listingInstance, field: 'endTime', 'error')} required">
	<label for="endTime">
		<g:message code="listing.endTime.label" default="End Time" />
		<span class="required-indicator">*</span>
	</label>
	<g:datePicker name="endTime" precision="day"  value="${listingInstance?.endTime}"  />
</div>
